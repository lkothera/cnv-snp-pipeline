...
# -----
# Title: combine_gene_exports.py
# Author: John Phan (John.Phan@csra.com)
# -----

# combine exported bed files from conifer, and combine all exon values for each gene
	...
	for bed_file_name in dirlist:
		...
		try:
			while True:
				...
				if gene_name not in bed_data[sample_name]:
					bed_data[sample_name][gene_name] = []

				bed_data[sample_name][gene_name].append(cnv_value)

		except(StopIteration):
			bed_file.close()

	return (bed_data, gene_list)
...
def main():
	...
	(data, gene_names) = read_bed_files(in_dir)
	...
	for gene in sorted(gene_names):
		out_file.write(gene)
		for sample in sorted(data):
			out_file.write('\t'+str(sum(data[sample][gene])/len(data[sample][gene])))
		out_file.write('\n')
