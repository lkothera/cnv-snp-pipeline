...
`samtools view $bamfile -H >$outsam`;
open SAM, ">>", "$outsam" || die "Can't open outsam\n$!";
open BAM, "samtools view $bamfile | " || die "Can't open bam file\n$!";
while (<BAM>)
{
	chomp;
	my @d = split (/\t/,$_);
...
open BAMS, "samtools view $bamfile | " || die "Can't open bam file\n$!";
while (<BAMS>)
{
	chomp;
	my @d = split (/\t/,$_);
...
`samtools view -S $outsam -b -o $outbam`;
`samtools sort -O bam -T tmp $outbam > $outsorted_bam`;
`samtools index $outsorted_bam`;
`rm $outsam $outbam`;
