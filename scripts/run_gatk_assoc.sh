...
${BASE}/scripts/assoc_variants_new_tests.py --vcf ${BASE}/pipeline-runs/${RUN_NAME}/gatk/variants.snp.vcf --compare ${BASE}/samples/${SAMPLES}/${COMPARE} --column ${COLUMN} --fasta ${BASE_REFERENCE}/bwa/supercontigs_combined_formatted_gatk.fa --gtf ${BASE_REFERENCE}/${REFERENCE}-combined_gatk.gtf --thresh 0.05 > ${BASE}/pipeline-runs/${RUN_NAME}/gatk_features/variants.snp_assoc_${NAME}.txt
